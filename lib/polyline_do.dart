...
class Polyline {
  ...
  /// .Encode(coords, p)
  Polyline.Encode({required this.decodedCoords, required this.precision}) {
    encodedString = _encode_poly(decodedCoords, precision);
    decodedCoords = decodedCoords;
  }

  // .Distance(str, u)
  Polyline.Distance({required this.encodedString, required this.unit}) {
    distance = _length(encodedString, unit); // _length sets decodedCoords
    encodedString = encodedString;
  }

  get NullThrownError => null;
